;; Private helper for ticket transfer and update
(define-private (do-ticket-transfer (event-id uint) (ticket-id uint) (owner principal) (new-buyer principal) (new-price uint))
  (begin
    (unwrap! (nft-transfer? ticket ticket-id owner new-buyer) (err ERR_PAYMENT_FAILED))
...
;; ---------------------------------------------------------------------------
;; Ticketing Contract with Resale & Max-Resale Logic
;; ---------------------------------------------------------------------------
  ...
  (let (
    ...
    (begin
      ...
      (let ((ticket-id (+ (get tickets-sold event) u1)))
        (unwrap! (nft-mint? ticket ticket-id buyer) (err ERR_PAYMENT_FAILED))
...
;; Resell a ticket to a new buyer (within max resale price)
(define-public (resell-ticket (event-id uint) (ticket-id uint) (new-price uint) (new-buyer principal))
  (let (
    ...
    (begin
      ...
      (asserts! (is-eq new-buyer new-buyer) (err ERR_INVALID_BUYER))
      (asserts! (is-eq new-price new-price) (err ERR_INVALID_PRICE))
